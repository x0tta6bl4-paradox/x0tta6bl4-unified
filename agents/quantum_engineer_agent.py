...
class QuantumEngineerAgent(BaseAgent):
    """Агент для исследований quantum supremacy"""

    def __init__(self):
        super().__init__("quantum_engineer_agent", "quantum_engineering")
        ...
        self.register_capability("quantum_supremacy_design", {
            "algorithms": ["shor", "grover", "qaoa", "vqe"],
            "max_problem_size": 1000
        })
        self.register_capability("quantum_metrics_measurement", {
            "metrics": ["coherence", "fidelity", "gate_errors", "entanglement"]
        })
        self.register_capability("research_coordination", {
            "collaborators": ["ai_engineer", "research_engineer"]
        })
    ...
    async def _execute_task_internal(self, task_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if task_name == "design_supremacy_demo":
            return await self.design_quantum_supremacy_demo(
                parameters.get("algorithm", "grover"),
                parameters.get("problem_size", 100)
            )
        elif task_name == "run_supremacy_demo":
            algorithm = parameters.get("algorithm", "grover")
            if algorithm == "shor":
                return await self.run_shor_demo(parameters.get("number", 15))
            elif algorithm == "grover":
                return await self.run_grover_demo(parameters.get("search_space", 64))
            elif algorithm == "qaoa":
                return await self.run_qaoa_demo(parameters.get("graph_size", 10))
            elif algorithm == "vqe":
                return await self.run_vqe_demo(parameters.get("molecule", "H2"))
        elif task_name == "measure_metrics":
            return await self.measure_quantum_metrics()
        elif task_name == "coordinate_research":
            return await self.coordinate_quantum_research(
                parameters.get("topic", "quantum_supremacy"),
                parameters.get("collaborators", [])
            )
    ...
    # Вспомогательные методы (оставлены без изменений для краткости)
    async def _update_quantum_metrics(self):
        ...
        try:
            ...
            metrics = QuantumMetrics(
                ...
                entanglement_fidelity=random.uniform(0.8, 0.99),
                gate_error_rate=random.uniform(0.001, 0.01),
                readout_error=random.uniform(0.01, 0.05),
                t1_time=random.uniform(20, 200),
                t2_time=random.uniform(15, 150),
                timestamp=time.time()
            )
    ...
    def _assess_scalability(self, algorithm: str, results: Dict) -> Dict[str, Any]:
        ...
        return {
            "current_scalability": problem_size / 100,
    ...
    async def _save_history(self):
        ...
        try:
            history_data = {
                ...
                "demos": [asdict(d) for d in self.demo_history[-50:]]
            }
